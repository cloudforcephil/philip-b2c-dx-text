...
/**
 * Resolve SLAS IdP name for the authorize `hint` parameter.
 * Uses app.login.social.idpHints when set; otherwise falls back to the button idp label.
 */
const getSlasHintForIdp = (idpButtonName, idpHints = {}) => {
    if (!idpButtonName) {
        return ''
    }
    if (idpHints[idpButtonName]) {
        return idpHints[idpButtonName]
    }
    const lower = idpButtonName.toLowerCase()
    if (idpHints[lower]) {
        return idpHints[lower]
    }
    const key = Object.keys(idpHints).find((k) => k.toLowerCase() === lower)
    return key ? idpHints[key] : idpButtonName
}
...
const SocialLogin = ({form, idps = []}) => {
    ...
    const socialConfig = getConfig()?.app?.login?.social || {}
    const redirectPath = socialConfig.redirectURI || ''
    const redirectURI = buildRedirectURI(appOrigin, redirectPath)
    const idpHints = socialConfig.idpHints || {}
    ...
    const onSocialLoginClick = async (name) => {
        try {
            ...
            const hint = getSlasHintForIdp(name, idpHints)
